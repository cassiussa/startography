	...
	void Start () {
		...
		for (int i=0; i<distanceMarkersName.Length; i++) {													// Iterate over each scale size that we'll have a Distance Marker for
			...
			for(int a=-1;a<=1;a++) {																			// So we can get a scale smaller, current, and larger
				//int layerMask = distanceMarkersLayer[i];													// Ge the integer of the layer from the distanceMarkersLayer array

				/*
				 * Somehow here I need to get the value just smaller than this layer
				 * and just larger than this layer.
				 */
				...
				if(thisLayer >= 8 && thisLayer <= 17) {														// Ensure that we're not going below the smallest layer or above the highest layer
					...
					distanceMarker.layer = thisLayer;														// Assign the layer based on the calculated layer
					...
					//distanceMarker.transform.parent = GameObject.Find ("/Galaxy/ScaleStates").transform;
					...
					distanceMarker.transform.position = gameObject.transform.position;							// Set the position of the newly-created gameObject to same location as that of the gameObject (star) that generated it
				}
			}
